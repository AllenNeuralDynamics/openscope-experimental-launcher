#!/usr/bin/env python3
"""
Test the session creator post-acquisition tool (flattened end_state schema).
"""
...
class TestSessionCreator:
    ...
    def test_load_experiment_data(self, tmp_path):
        ...
        assert creator.end_state["subject_id"] == "test_subject_123"
    ...
    def test_create_session_file(self, tmp_path):
        ...
        assert "test_user" in session_data.get("experimenter_full_name", "") or "test_user" in session_data.get("notes", "")
    ...
    def test_missing_files_graceful_handling(self, tmp_path):
        ...
        md = tmp_path / "launcher_metadata"; md.mkdir()
        with open(md / "end_state.json", "w") as f: json.dump({"subject_id": "test_subject", "start_time": "2024-01-01T10:00:00"}, f)
        creator = SessionCreator(str(tmp_path))
        assert creator.load_experiment_data() is True
        assert creator.end_state["subject_id"] == "test_subject"
        assert creator.launcher_metadata == {}
