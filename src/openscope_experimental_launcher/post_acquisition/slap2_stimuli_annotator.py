...
- Prompts operator to choose which .harp folder (and therefore behavior session) to use.
- Moves all CSVs (including orientations_*.csv) into behavior/stimuli/.
...
def run(params: Dict[str, Any]) -> int:
    if params is None:
        ...
        LOG.warning("slap2_stimuli_annotator received non-dict params; interpreting as session dir")
        params = {"output_session_folder": params}

    session_dir_param = params.get("output_session_folder")
    if not session_dir_param:
        LOG.error("output_session_folder is required for slap2_stimuli_annotator")
        return 2
