    ...
    try:
        ...
        while process.poll() is None:
            try:
                ...
                if current_memory_percent > initial_memory_percent + kill_threshold:
                    logging.warning(
                        f"Detected runaway process, memory usage: {current_memory_percent}% "
                        f"(threshold: {initial_memory_percent + kill_threshold}%)"
                    )
                ...
                if time.time() - last_memory_log > 60:  # Log every minute
...
def get_process_memory_info(process: subprocess.Popen) -> dict:
    """
...
def is_process_responsive(process: subprocess.Popen, timeout: float = 5.0) -> bool:
    """
